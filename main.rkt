...
(define vetor-string-palavra4 (make-vector (string-length palavra_nivel4) "_ " ))
...
(define palavra_atual (vector-ref vetor_nivel1 (random 9)))
(define vetor-string-palavra-atual (make-vector (string-length palavra_atual) "_ " ))

(define lista_palavras (list palavra_nivel2 palavra_nivel3 palavra_nivel4))
(define vetor-string_palavras (list vetor-string-palavra2 vetor-string-palavra3 vetor-string-palavra4))
...
(define msg-text2_passou_de_fase (new message% [parent frame_passou_de_fase] [label (string-append "A palavra era " palavra_atual)] [stretchable-width #t]))
(define button_passou_de_fase (new button% [parent frame_passou_de_fase] [label "Obrigado!"]
     [callback (lambda (button event)
                 (send frame_passou_de_fase show #f)
                 (forca))])) ;O PROGRAMA TERMINA
